...
Определите переменные окружения, для этого надо создать файл `.env` в каталоге `star_burger/` и положить туда такой код:
```sh
...
- `YANDEX_API_KEY` - ключ, который отвечает за определение координат мест. Нужно получить от Яндекс Геокодера самим.
